...
# ---------- add ----------
printf 'sk-test-token-12345\n' | sc add NPM_TOKEN --tag npm,ci >/dev/null
printf 'ghp_test-token-67890\n' | sc add GITHUB_TOKEN --tag github >/dev/null
echo "ok: add"

# ---------- list ----------
LIST_JSON="$(sc list --json)"
...
cd "$PROJECT"
ENV_OUT="$(sc exec --tag npm -- env | grep '^NPM_TOKEN=' || true)"
...
set +e
sc exec --tag npm -- /this/does/not/exist
EC=$?
set -e
...
# allowed
sc exec --tag npm -- env >/dev/null
echo "ok: policy allows env+npm"

# disallowed command
set +e
sc exec --tag npm -- sh -c 'echo bad'
EC=$?
set -e
...
EOF
sc render "$PROJECT/npmrc.tmpl" --out "$PROJECT/.npmrc" >/dev/null
...
# ---------- rm ----------
sc rm GITHUB_TOKEN >/dev/null
LIST_AFTER="$(sc list --json)"
...
EDITOR="$WRITE_NEW" sc add MULTILINE --tag editor --editor >/dev/null
REVEAL="$(sc reveal MULTILINE)"
case "$REVEAL" in
  *"multi-line"*)
    case "$REVEAL" in
      *"secret-via-editor"*) ;;
...
EOF
chmod +x "$REPLACE"
EDITOR="$REPLACE" sc edit MULTILINE >/dev/null
REVEAL3="$(sc reveal MULTILINE)"
case "$REVEAL3" in
...
TAGS_AFTER_EDIT="$(sc list --json | grep -o '"name":"MULTILINE"[^}]*')"
...
chmod +x "$EMPTY"
set +e
EDITOR="$EMPTY" sc add EMPTY_SECRET --editor 2>/dev/null
EC=$?
set -e
[[ $EC -eq 1 ]] || { echo "FAIL: empty editor add should exit 1, got $EC"; exit 1; }
LIST_FINAL="$(sc list --json)"
...
sc materialize NPM_TOKEN --out "$TOKEN_OUT" >/dev/null
...
sc materialize NPM_TOKEN --out "$TOKEN_OUT.0644" --mode 0644 >/dev/null
...
sc materialize NPM_TOKEN --out "$NESTED" --mkdir >/dev/null
...
set +e
sc materialize NOT_A_SECRET --out "$WORK/x" 2>/dev/null
EC=$?
set -e
